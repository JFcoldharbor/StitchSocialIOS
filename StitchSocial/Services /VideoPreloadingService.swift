...
extension VideoPreloadingService {
    ...
    /// Test preload with sample video
    func testPreload() async {
        print("🧪 PRELOAD: Testing preload functionality...")
        ...
        // Test preload
        await preloadVideo(testVideo, priority: .high)
        
        // Check result
        let status = getPreloadStatus(for: testVideo.id)
        switch status {
        case .ready:
            print("✅ PRELOAD: Test passed - video ready for playback")
        case .preloading(let progress):
            print("⏳ PRELOAD: Test in progress - \(String(format: "%.1f%%", progress * 100))")
        default:
            print("❌ PRELOAD: Test failed - \(status)")
        }
